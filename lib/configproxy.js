...
export class ConfigurableProxy extends EventEmitter {
  ...
  reloadSsl(callback) {
    // reload SSL certificates and keys from disk
    // enables zero-downtime cert renewal (e.g., Let's Encrypt)
    var that = this;

    try {
      // reload main SSL options from disk
      if (this.options._sslKeyFile || this.options._sslCertFile) {
        var newSslOpts = {};

        if (this.options._sslKeyFile) {
          newSslOpts.key = fs.readFileSync(this.options._sslKeyFile);
          if (this.options._sslPassphrase) {
            newSslOpts.passphrase = this.options._sslPassphrase;
          }
        }
        if (this.options._sslCertFile) {
          newSslOpts.cert = fs.readFileSync(this.options._sslCertFile);
        }
        if (this.options._sslCaFile) {
          newSslOpts.ca = fs.readFileSync(this.options._sslCaFile);
        }
        if (this.options._sslDhparamFile) {
          newSslOpts.dhparam = fs.readFileSync(this.options._sslDhparamFile);
        }
        if (this.options._sslProtocol) {
          newSslOpts.secureProtocol = this.options._sslProtocol + "_method";
        }
        newSslOpts.ciphers = this.options._sslCiphers;
        newSslOpts.honorCipherOrder = true;
        newSslOpts.requestCert = this.options._sslRequestCert;
        newSslOpts.rejectUnauthorized = this.options._sslRejectUnauthorized;

        this.options.ssl = newSslOpts;
      }

      // reload API SSL options from disk
      if (this.options._apiSslKeyFile || this.options._apiSslCertFile) {
        var newApiSslOpts = {};

        if (this.options._apiSslKeyFile) {
          newApiSslOpts.key = fs.readFileSync(this.options._apiSslKeyFile);
          if (this.options._apiSslPassphrase) {
            newApiSslOpts.passphrase = this.options._apiSslPassphrase;
          }
        }
        if (this.options._apiSslCertFile) {
          newApiSslOpts.cert = fs.readFileSync(this.options._apiSslCertFile);
        }
        if (this.options._apiSslCaFile) {
          newApiSslOpts.ca = fs.readFileSync(this.options._apiSslCaFile);
        }
        if (this.options._sslDhparamFile) {
          newApiSslOpts.dhparam = fs.readFileSync(this.options._sslDhparamFile);
        }
        if (this.options._sslProtocol) {
          newApiSslOpts.secureProtocol = this.options._sslProtocol + "_method";
        }
        newApiSslOpts.ciphers = this.options._sslCiphers;
        newApiSslOpts.honorCipherOrder = true;
        newApiSslOpts.requestCert = this.options._apiSslRequestCert;
        newApiSslOpts.rejectUnauthorized = this.options._apiSslRejectUnauthorized;

        this.options.apiSsl = newApiSslOpts;
      }

      // swap the running proxy server with one using the new SSL options
      if (this.options.ssl && this.proxyServer) {
        var logErrors = this.proxyServer.listeners("request")[0];
        var newServer = https.createServer(
          { ...this.options.ssl, keepAlive: true, keepAliveTimeout: this.options.keepAliveTimeout || 5000 },
          logErrors
        );
        newServer.on("upgrade", this.handleProxyWs.bind(this));

        var oldProxyServer = this.proxyServer;
        var proxyAddr = oldProxyServer.address();

        this.proxyServer = newServer;
        newServer.listen(proxyAddr.port, proxyAddr.address, function () {
          oldProxyServer.close(function () {
            that.log.info("Old proxy server closed after SSL reload");
      ...
      if (this.options.apiSsl && this.apiServer) {
        var apiCallback = this.apiServer.listeners("request")[0];
        var newApiServer = https.createServer(this.options.apiSsl, apiCallback);

        var oldApiServer = this.apiServer;
        var apiAddr = oldApiServer.address();

        this.apiServer = newApiServer;
        newApiServer.listen(apiAddr.port, apiAddr.address, function () {
          oldApiServer.close(function () {
            that.log.info("Old API server closed after SSL reload");
          });
        });
      }
    } catch (e) {
      that.log.error("Error during SSL reload: " + e.message);

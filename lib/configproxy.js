...
class ConfigurableProxy extends EventEmitter {
  constructor(options) {
    ...
    this.proxy.on("proxyRes", function (proxyRes, req, res) {
      ...
      this.metrics.requestsCount.labels(proxyRes.statusCode).inc();
    });
  }
  ...
  getRoutes(req, res, path) {
    ...
    return this._routes.getAll().then((routes) => {
      ...
      this.metrics.apiRouteGetCount.inc();
    });
  }
  ...
  handleProxyError(code, kind, req, res, e) {
    ...
    this.metrics.requestsCount.labels(code).inc();
    if (e) {
  ...
  handleProxyWeb(req, res) {
    ...
    if (!this.options.disableMetrics && this.handleMetrics(req, res)) {
      return;
    }
    // Proxy a web request
  ...
  handleMetrics(req, res) {
    if (req.url === "/metrics") {
      return this.metrics.render().then((s) => {
  ...
  handleApiRequest(req, res) {
    ...
    this.metrics.requestsApiCount.inc();
    if (res) {
      res.on("finish", () => {

...
class ConfigurableProxy extends EventEmitter {
  constructor(options) {
    ...
    this.proxy.on("proxyRes", function (proxyRes, req, res) {
      ...
      metrics.requestsCount.labels(proxyRes.statusCode).inc();
    });
  }
  ...
  deleteRoutes(req, res, path) {
    ...
    return this._routes.get(path).then((result) => {
      ...
      return p.then(() => {
        ...
        metrics.apiRouteDeleteCount.inc();
      });
    });
  }
  ...
  targetForReq(req) {
    ...
    var metricsTimerEnd = metrics.findTargetForReqSummary.startTimer();
  ...
  updateLastActivity(prefix) {
    ...
    var metricsTimerEnd = metrics.lastActivityUpdatingSummary.startTimer();
    var routes = this._routes;
  ...
  handleProxyError(code, kind, req, res, e) {
    ...
    metrics.requestsCount.labels(code).inc();
    if (e) {
  ...
  handleProxyWeb(req, res) {
    ...
    if (this.handleMetrics(req, res)) {
      return;
    }
    // Proxy a web request
    this.statsd.increment("requests.web", 1);
    metrics.requestsWebCount.inc();

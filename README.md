...
Options:
  ...
  --storage-backend <storage-class>  Define a route storage backend: the built-in 'redis' store (set REDIS_URL), or an external storage class. Defaults to in-MemoryStore.
...
To persist routes across restarts, select the built-in Redis store with
`--storage-backend redis` and point the proxy at a Redis server via the
`REDIS_URL` environment variable:

```bash
export REDIS_URL=redis://localhost:6379   # use rediss:// for TLS
configurable-http-proxy --storage-backend redis
```
...
- `REDIS_URL` is required when `--storage-backend redis` is used; the proxy
  exits with an error if it is unset. Supported URL forms are `redis://` and
  `rediss://` (TLS).
